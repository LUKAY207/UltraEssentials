...
class TPACommand extends Command implements PluginOwned {

    public function __construct(string $name, Translatable|string $description = "", Translatable|string|null $usageMessage = null, array $aliases = []) {
        $this->setPermission('ultraessentials.tpa.command');
    ...
    public function execute(CommandSender $sender, string $commandLabel, array $args) {
        ...
        if (!$sender->hasPermission('ultraessentials.tpa.command')) {
            $sender->sendMessage($loader->translate('command-executor-no-permission', $loader->getPrefix()));
            return;
        }
        if (empty($args[0])) {
            $sender->sendMessage($loader->translate('command-tpa-usage-message', $loader->getPrefix()));
            return;
        }
        ...
        $teleportManager->send($sender, $teleportSubject);
        $sender->sendMessage($loader->translate('message-sent-tpa', $loader->getPrefix(), $teleportSubject));
        $teleportSubject->sendMessage($loader->translate('message-sent-tpa-target', $loader->getPrefix(), $sender));
    }
